...
import time
    ...
    if __name__ == '__main__':
        ...
        img_rgb = cv2.imread('C:/Users/20180918/Desktop/image/buffer2.bmp')  # 需要检测的图片
        img_gray = cv2.cvtColor(img_rgb, cv2.COLOR_BGR2GRAY)  # 转化成灰色
        template_img = cv2.imread('C:/Users/20180918/Desktop/image/buffer1.bmp', 0)  # 模板小图
        ...
        for coord in dets:
            ...
            sum = cut(img_rgb,int(left[0][0]),int(left[0][1]),int(right[0][0]),int(right[0][1]),i)
            sum1 = sum1 + sum
